...
**listUserBorrows(db, { userId, now })** → **2 次查詢**:
...
2. `SELECT l.book_id, b.title, l.borrowed_at, l.due_at, l.returned_at FROM loans l JOIN books b ON b.id=l.book_id WHERE l.user_id=? ORDER BY l.borrowed_at DESC`
   ...
   每筆補 `overdue_days`:active 用 `overdueDays(row.due_at, now)`,history 用 `overdueDays(row.due_at, row.returned_at)`;同一支純函式,`now` 由 routes 傳進來。
...
export function createApp(clock = () => new Date().toISOString()) {
  ...
  app.get('/api/users/:id/borrows', …)  // now = clock(),傳給 listUserBorrows 算 active 段的 overdueDays
  return app;
}
```
...
- borrows 200 `{ active: [{ bookId, title, borrowedAt, dueAt, returnedAt: null, overdueDays }], history: [{ …, returnedAt, overdueDays }] }`
  (active 的 `overdueDays` 以 `now` 算,history 的以 `returnedAt` 算)
...
- GET borrows:active/history 分段正確、順序、404;**`counter` 斷言恰好 2 次 SQL**;
  `overdueDays`:clock 釘在 due 後 3 天 → active 那筆 = 3;history 那筆 = 以 `returnedAt` 算(還書時逾期 1 天,之後 clock 再往後撥也仍是 1)
...
devlog、罰款、預約、建書/建人 endpoint、presentation 層、git commit。
