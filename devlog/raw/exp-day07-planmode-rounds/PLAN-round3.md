...
- Schema 6 張:`readers`(含 `role`)、`loan_policies`(一列,`loan_days` 與 `hold_hours` 都存這裡)、`books`、`copies`、`loans`(含反正規化的 `book_id`)、`reservations`
...
- 同一讀者對同一本書:只能有一筆進行中的借閱,也只能排一次隊;**已借此書的人不能再排同一本**(409)
- `DELETE /reservations/:id` 遇到 `held` 也允許取消:副本順延給下一位隊首,沒人才 `available`
- 政策值(14 天 / 48 小時)在借出 / 建立保留當下算好快照進 `loans.due_at` / `reservations.hold_expires_at`,之後只看快照
...
- `src/domain/loan-policy.js` —— `dueAtFor(nowIso, loanDays)`、`holdExpiresAtFor(nowIso, holdHours)`。只用 `Date.parse` + `new Date(ms).toISOString()`,**不讀時鐘**;`loanDays` / `holdHours` 由 route / worker 從 `loan_policies` 讀出後當參數傳入。
...
loan_policies (id INTEGER PK CHECK (id = 1), loan_days INTEGER NOT NULL, hold_hours INTEGER NOT NULL)
                -- 單列;0001 migration 內 INSERT (1, 14, 48)
...
route 先 `SELECT loan_days, hold_hours FROM loan_policies WHERE id = 1`(所有寫入類 route 與 `scheduled()` 都這樣開頭),`dueAt = dueAtFor(now, loan_days)`、`holdExpiresAt = holdExpiresAtFor(now, hold_hours)`,算好才往下傳;之後只看寫進去的快照。
...
WHERE EXISTS (books WHERE id=?)
  ...
  AND NOT EXISTS (loans WHERE reader_id=? AND book_id=? AND returned_at IS NULL)   -- 已借此書不能再排
```
...
- `tests/domain/loan-policy.test.js`:`dueAtFor('2026-09-17T00:00:00.000Z', 14)` = `2026-10-01T00:00:00.000Z`;`holdExpiresAtFor(now, 48)` = +2d;跨月、跨年;輸出永遠以 `Z` 結尾;兩個函式都不讀時鐘(同一輸入呼叫兩次結果相同)。
...
- `tests/db/return.test.js`:無隊列 → `available`;有隊列 → 隊首 `held`、`hold_expires_at = now + loan_policies.hold_hours`、副本 `held`;館員可還別人的;非本人非館員 → 403;重複歸還 → 409;`overdue_days` 用快照(歸還前把 `loan_policies.loan_days` 改成 1,結果不變)。
- `tests/db/reserve.test.js`:有可借 → 409;全借出 → 201 `waiting`;重複 → 409;已借此書 → 409。
- `tests/db/cancel.test.js`:`waiting` → 204;`held` → 204 且副本順延下一位 / 無人則 `available`;非本人 → 403;已 fulfilled → 409。
- `tests/db/expire.test.js`:`now < expires` 不動;`now >= expires` → `expired`、順延下一位;無人排隊 → `available`;重跑冪等;判斷只看 `hold_expires_at` 快照(先把 `loan_policies.hold_hours` 改成 1 再跑,結果不變)。
